...
export default function WageRatesSettingsPage() {
  ...
  const fetchPresets = async () => {
    try {
      ...
      if (res.ok) {
        ...
        data.sort((a: any, b: any) => (a.workerType || a.name).localeCompare(b.workerType || b.name));
        setPresets(data);
  ...
  return (
    <div className="space-y-6 max-w-5xl py-6 px-4 md:px-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-900">Worker Types & Wage Rates</h2>
          <p className="text-muted-foreground mt-1 text-sm">Configure daily wage rates and payment cycles (Daily vs Weekly) for each worker category.</p>
        </div>
        <Button 
          onClick={() => {
      ...
      <Card className="border-slate-200 shadow-sm bg-slate-50/50">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-bold text-slate-900">Administrative Bulk Creation</CardTitle>
          <CardDescription className="text-xs">
            Directly create new official categories of trade or labour to be made immediately available in project logs and daily entry forms.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleCreateNewType} className="flex flex-col sm:flex-row gap-4 items-end">
            <div className="flex-1 space-y-1 w-full">
              <label className="text-xs font-semibold text-slate-700">Worker Type Name *</label>
              <Input
            ...
            <div className="w-full sm:w-[160px] space-y-1">
              <label className="text-xs font-semibold text-slate-700">Payment Cycle</label>
              <select
            ...
            <Button type="submit" disabled={creating} className="w-full sm:w-auto font-medium shadow-xs">
              {creating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-1.5 h-4 w-4" />}
              Add New Worker Type
            </Button>
          </form>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            These settings pre-fill the Daily Labour form. Selecting a DAILY cycle trade (like Helper) defaults "Paid on Spot" to checked; WEEKLY trades default to unchecked.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="lg:hidden space-y-3.5">
            {loading ? (
              ...
              presets.map((preset) => {
                ...
                return (
                  <div key={typeName} className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-sm hover:border-slate-300 transition-all space-y-3.5">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                      <h3 className="font-bold text-slate-900 text-base break-words">{typeName}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-1">
                      <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-600 uppercase tracking-wider block">Daily Rate (₹)</label>
                        <Input 
                          type="number" 
                          min="0"
                          step="0.01"
                          ...
                          onChange={(e) => setEditingValues(prev => ({ ...prev, [typeName]: e.target.value }))}
                          onKeyDown={(e) => {
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-600 uppercase tracking-wider block">Payment Cycle</label>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setEditingCycles(prev => ({ ...prev, [typeName]: newCycle }));
                          }}
                        >
                          <option value="WEEKLY">Weekly</option>
                          <option value="DAILY">Daily</option>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex justify-end">
                      ...
                      >
                        {isSaving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Check className="h-4 w-4 mr-1.5" />}
          ...
          <div className="hidden lg:block rounded-xl border bg-white shadow-sm overflow-hidden">
            <Table className="min-w-[600px]">
              <TableHeader className="bg-slate-50/80 border-b border-slate-200">
                <TableRow>
                  <TableHead className="w-[240px] font-semibold text-slate-700">Worker Type</TableHead>
                  <TableHead className="w-[220px] font-semibold text-slate-700">Default Daily Rate (₹)</TableHead>
                  <TableHead className="w-[180px] font-semibold text-slate-700">Payment Cycle</TableHead>
                  <TableHead className="w-[120px] text-right font-semibold text-slate-700">Action</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-12 text-muted-foreground">Loading worker types...</TableCell>
                  </TableRow>
                ) : (
                  presets.map((preset) => {
                    const typeName = preset.workerType || preset.name;
                    return (
                      <TableRow key={typeName} className="hover:bg-slate-50/60 transition-colors">
                        <TableCell className="font-semibold text-slate-800">{typeName}</TableCell>
                        <TableCell>
                          <Input 
                            type="number" 
                            min="0"
                        ...
                        <TableCell className="text-right">
                          ...
                          >
                            {savingMap[typeName] ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Check className="h-4 w-4 mr-1" />}
