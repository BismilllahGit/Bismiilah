...
export default function DailyLabourPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const projectId = resolvedParams.id;

  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  const [entries, setEntries] = useState<any[]>([]);
  const [summary, setSummary] = useState<any>({ totalHeadcount: 0, totalSpend: 0 });
  const [loading, setLoading] = useState(false);
  
  const [presets, setPresets] = useState<Record<string, { defaultRate: number; paymentCycle: string }>>({});
  ...
  const fetchEntries = async () => {
    if (!projectId) return;
    setLoading(true);
    const res = await fetch(`/api/projects/${projectId}/daily-labour?startDate=${date}&endDate=${date}&limit=1000`);
    if (res.ok) {
      const data = await res.json();
  ...
  const handleCreateInlineWorkerType = async (rowId: string, name: string) => {
    ...
    try {
      const res = await fetch("/api/worker-types", {
        ...
        body: JSON.stringify({ name: name.trim(), defaultRate: 0, paymentCycle: "WEEKLY" })
      });
      const data = await res.json();
      if (res.ok || res.status === 201) {
        ...
        alert(data.error?.name?._errors?.[0] || data.error || "Failed to create worker type");
        updateRow(rowId, "creatingCustom", false);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate rows
    const validRows = formRows.filter(r => r.workerType && r.workerType !== "__OTHERS__" && Number(r.headcount) > 0 && Number(r.wageRate) > 0);
    ...
    try {
      ...
      if (res.ok) {
        ...
        const types = Array.from(new Set(payload.entries.map(e => e.workerType)));
        
        // e.g., "Logged 10 Masons and 6 Helpers — Total: ₹18,300 for 14 June"
        setSuccessMessage(`Logged ${result.totalHeadcount} workers (${types.join(", ")}) — Total: ₹${result.totalSpend.toLocaleString('en-IN')} for ${new Date(formDate).toLocaleDateString()}`);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Log and track daily headcount-based labour expenses.</p>
        </div>
              ...
              {successMessage ? (
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    {formRows.map((row) => (
                      <div key={row.id} className="grid grid-cols-12 gap-2 p-3 bg-slate-50 border rounded-md relative items-start">
                        <div className="col-span-12 sm:col-span-4 space-y-1">
                          <label className="text-xs font-medium text-slate-500">Worker Type *</label>
                          <select 
                            required
                            className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm"
                            value={row.workerType}
                            onChange={(e) => updateRow(row.id, "workerType", e.target.value)}
                          >
                          ...
                          {row.workerType === "__OTHERS__" && (
                            <div className="flex items-center gap-1 pt-1">
                              <Input 
                                ...
                                onChange={(e) => updateRow(row.id, "customTypeName", e.target.value)}
                                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleCreateInlineWorkerType(row.id, row.customTypeName || ""); } }}
                              ...
                              <Button 
                                type="button" 
                                size="sm" 
                                disabled={row.creatingCustom || !row.customTypeName?.trim()}
                                onClick={() => handleCreateInlineWorkerType(row.id, row.customTypeName || "")}
                              ...
                              <Button 
                                ...
                                variant="ghost" 
                                size="sm" 
                                onClick={() => updateRow(row.id, "workerType", "")}
                        ...
                        <div className="col-span-6 sm:col-span-4 space-y-1">
                          ...
                          <Input 
                            ...
                            placeholder="e.g. 5"
                            value={row.headcount}
                            onChange={(e) => updateRow(row.id, "headcount", e.target.value)}
                          />
                        </div>
                        <div className="col-span-6 sm:col-span-4 space-y-1">
                          <label className="text-xs font-medium text-slate-500">Wage (₹) *</label>
                          <Input 
                            required
                            type="number"
                            min="0.01"
                            ...
                            value={row.wageRate}
                            onChange={(e) => updateRow(row.id, "wageRate", e.target.value)}
                          />
                        </div>
                        <div className="col-span-12 sm:col-span-12 space-y-2 mt-1">
                          <label className="text-xs font-medium text-slate-500">Task & Contractor (Optional)</label>
                          <div className="flex flex-col sm:flex-row gap-2">
                            ...
                            <select 
                              ...
                              onChange={(e) => updateRow(row.id, "contractorId", e.target.value)}
                              className="flex h-9 w-full sm:w-1/3 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm"
                            >
                          ...
                          <div className="flex items-center space-x-2 pt-1">
                            <input
                              ...
                              onChange={(e) => updateRow(row.id, "paidImmediately", e.target.checked)}
                              className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                            />
                            <label htmlFor={`paid-${row.id}`} className="text-xs font-medium text-slate-700 cursor-pointer">
                              Paid Immediately (Settled on spot in cash, exclude from contractor balance)
                            </label>
                          </div>
                        </div>
                        {formRows.length > 1 && (
                          <Button 
                            ...
                            variant="ghost" 
                            size="icon" 
                            className="absolute -right-2 -top-2 h-6 w-6 rounded-full border bg-background text-destructive hover:bg-destructive/10 shadow-sm"
          ...
          <>
            <div className="space-y-3.5">
              {entries.map((entry: any) => (
                <div key={entry.id} className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-sm hover:border-slate-300 transition-all space-y-3">
                  <div className="flex items-start justify-between gap-2 border-b border-slate-100 pb-2.5">
                    <span className="font-bold text-slate-900 text-base break-words">{entry.workerType}</span>
                  ...
                  <div className="space-y-1">
                    <div className="font-semibold text-sm text-slate-800 break-words">{entry.title || "Daily Labour"}</div>
                    {(entry.contractorName || entry.broughtBy) && (
                      <div className="text-xs font-medium text-slate-500">Contractor: <span className="text-slate-700 font-semibold">{entry.contractorName || entry.broughtBy}</span></div>
                    )}
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-100 text-xs">
                    <div className="bg-slate-50/80 rounded-lg p-2 text-center border border-slate-100/80 flex flex-col justify-center">
                      ...
                      <span className="font-mono font-bold text-slate-900 text-sm mt-0.5">{entry.headcount} Workers</span>
                    </div>
                    <div className="bg-slate-50/80 rounded-lg p-2 text-center border border-slate-100/80 flex flex-col justify-center">
                      <span className="text-slate-500 text-[10px] uppercase font-semibold">Wage Rate</span>
                      <span className="font-mono font-semibold text-slate-700 text-sm mt-0.5">₹{Number(entry.wageRate).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                    </div>
                    <div className="bg-slate-50/80 rounded-lg p-2 text-center border border-slate-100/80 flex flex-col justify-center">
                      <span className="text-slate-500 text-[10px] uppercase font-semibold">Total Spend</span>
                      <span className="font-mono font-bold text-slate-950 text-sm sm:text-base mt-0.5">₹{Number(entry.totalSpend).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-slate-900 text-white rounded-xl p-4 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-2 border border-slate-800">
              <div className="flex items-center justify-between sm:justify-start gap-4">
                <span className="uppercase text-xs font-bold tracking-wider text-slate-300">Day Total Workers</span>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-4 border-t border-slate-800 sm:border-0 pt-2 sm:pt-0">
                <span className="uppercase text-xs font-bold tracking-wider text-slate-300">Day Total Spend</span>
                <span className="font-mono text-lg sm:text-xl font-bold text-emerald-400">₹{summary.totalSpend?.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
              </div>
            </div>
          </>
        )}
      ...
      <div className="hidden lg:block border rounded-xl bg-white shadow-sm overflow-hidden">
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-10 text-muted-foreground">Loading labour entries...</TableCell>
              </TableRow>
            ) : entries.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-10 text-muted-foreground">No labour logged on this date.</TableCell>
              </TableRow>
            ) : (
              entries.map((entry: any) => (
                <TableRow key={entry.id}>
                  <TableCell className="font-medium">
                    {entry.workerType}
                    <div className="text-[10px] text-muted-foreground uppercase">{entry.voucherNumber}</div>
                  </TableCell>
                  <TableCell>
                    <div className="font-medium text-sm">{entry.title || "-"}</div>
                    {(entry.contractorName || entry.broughtBy) && <div className="text-xs text-muted-foreground">Contractor: {entry.contractorName || entry.broughtBy}</div>}
                    {entry.paidImmediately && <Badge variant="secondary" className="text-[10px] font-medium mt-0.5">Paid on Spot</Badge>}
                  </TableCell>
                  <TableCell className="text-right">{entry.headcount}</TableCell>
                  <TableCell className="text-right">{Number(entry.wageRate).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</TableCell>
                  <TableCell className="text-right font-medium">
                    {Number(entry.totalSpend).toLocaleString('en-IN', { maximumFractionDigits: 2 })}
                  </TableCell>
                </TableRow>
              ))
            )}
          ...
          {!loading && entries.length > 0 && (
            <tfoot className="bg-slate-50 font-bold border-t">
              <TableRow>
                <TableCell colSpan={2} className="text-right uppercase text-xs tracking-wider">Day Total</TableCell>
                <TableCell className="text-right">{summary.totalHeadcount}</TableCell>
                <TableCell></TableCell>
                <TableCell className="text-right">₹{summary.totalSpend?.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</TableCell>
              </TableRow>
            </tfoot>
          )}
        </Table>
